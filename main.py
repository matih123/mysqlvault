...
class MainApp(App):
    ...
    # Simulate key press on windows to automatically insert text
    def keypress(self, element, widget):
        if platform == 'win':
            keyboard = Controller()
            sleep(2)
    ...
    def checkbox_default_server_click(self, checkbox):
        try:
            ...
            if self.checkbox_default_server.active:
                self.b1_input.text = lines[0]
                self.b1_input.disabled = True
                self.b2_input.text = lines[1]
                self.b2_input.disabled = True
                self.b3_input.text = lines[2]
                self.b3_input.disabled = True
                ...
                self.b3_input.background_color = (.564, .729, 1, 1)
                self.b3_input.foreground_color = (0, 0, 0, 1)
                ...
                self.b3_input.background_color = (0, 0, 0, 0)
                self.b3_input.foreground_color = (1, 1, 1, 1)
        except:
            pass
    ...
    def show_elements(self, searched=''):
        ...
        if platform == 'win': 
            self.button_password_insert = dict()
            self.button_login_insert = dict()
        ...
        for i, p in enumerate(self.passwords):
            if searched in p[1]:
                ...
                if platform == 'win': self.grid_subbox[i] = GridLayout(cols=4, size_hint=(.8, 1), spacing='3dp')
                else: self.grid_subbox[i] = GridLayout(cols=3, size_hint=(.8, 1), spacing='3dp')
                ...
                self.button_login_copy[i] = Button(
                                ...
                                font_size='18dp',
                                bold=True)
                if platform == 'win': self.button_login_insert[i] = Button(
                                background_color=(.564, .729, 1, 1),
                                color=(1,1,1,1),
                                background_normal='',
                                text='In',
                ...
                self.label_password[i] = ElementTextInput(
                                ...
                                font_size='16dp',
                                padding='7dp')
                if platform == 'win': self.button_password_insert[i] = Button(
                                background_color=(.564, .729, 1, 1),
                                color=(1,1,1,1),
                                background_normal='',
                                text='In',
                ...
                if platform == 'win': 
                    self.button_login_insert[i].bind(on_press = partial(self.keypress, self.label_login[i]))
                    self.button_password_insert[i].bind(on_press = partial(self.keypress, self.label_password[i]))
                ...
                if platform == 'win': self.grid_subbox[i].add_widget(self.button_login_insert[i])
                ...
                if platform == 'win': self.grid_subbox[i].add_widget(self.button_password_insert[i])
    ...
    def set_config(self):
        ...
        try:
            # Config in Android
            if platform == 'android':
                self.default_file = f'{os.path.dirname(os.path.abspath(__file__))}\mysqlvault.txt'

            # Config in Windows
            elif platform == 'win':
                default_dir = f'{os.path.expanduser("~")}\.mysqlvault'
                if not os.path.exists(default_dir):
                    os.makedirs(default_dir)
                self.default_file = f'{default_dir}\mysqlvault.txt'
